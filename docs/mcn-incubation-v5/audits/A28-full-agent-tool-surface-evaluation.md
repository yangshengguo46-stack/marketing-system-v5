...
新入口已经由离线测试证明能准备项目、运行事件收集器并密封证据；本轮没有执行新的真实付费模型 trial，尚不能判断模型是否会正确使用三个知识工具，更不能判断孵化结果是否改善。
...
- `--max-agent-steps` 限制每个 LangGraph trial 的递归步数；
- `--execute` 明确确认会产生模型费用。
...
- 未执行真实模型运行，尚无实际工具选择、最终判断、成本或延迟数据。
