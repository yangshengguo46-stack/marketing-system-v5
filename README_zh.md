...
> 方法检索也会携带已复核的来源类型、适用范围、支持主张、限制和复核日期；平台指南只是有范围的证据，不会被当成通用增长公式。
> 项目证据现已采用独立的 Owner 隔离追加式链路：Lead Agent 能读取快照来源、哈希、范围、局限、争议和过期提醒，但证据不会自动升级为事实，浏览器凭据也不会进入模型上下文。
> 下一条评测链已经能让现有 Lead Agent 读取隔离的版本化项目，并密封配置工具面、脱敏后的实际工具轨迹、最终回答、用量和数据库哈希。目前仅通过离线测试，没有新增付费 trial，也没有据此宣称孵化质量已经提升。
