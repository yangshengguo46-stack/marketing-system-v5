...
> [!NOTE]
> The current marketing-brain checkpoint keeps six evaluation utilities under `backend/scripts/`, including isolated one-call and two-step layered-map evaluators. They require explicit `--execute` and write redacted artifacts under `.deer-flow/`. The default Lead can invoke two bounded capabilities on demand: `analyze_business_semantics` explicates what the user does, while `explore_content_worlds` expands one fact-bounded map of what that business can discuss long term. Business-operation design is outside both capabilities; the overall Lead may address it separately when the user's request requires it. The one-call candidate passed `1/6`; freezing the four-layer skeleton before expansion improved this to `3/6`, but still failed the preregistered threshold and the core golden-gift semantic leap. Neither candidate is registered, and production runtime source remains identical to commit `722919f7`. Neither capability is middleware, a mandatory incubation stage, or the final decision-maker. See [`A34`](docs/mcn-incubation-v5/audits/A34-production-content-world-exploration.md), [`A44`](docs/mcn-incubation-v5/audits/A44-two-step-content-map-preregistration.md), and [`ADR-010`](docs/mcn-incubation-v5/decisions/ADR-010-reject-two-step-content-map-promotion.md).
