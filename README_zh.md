...
> [!IMPORTANT]
> 这个工作区是第五版 MCN 孵化分支。DeerFlow 现有的唯一 Lead Agent 直接负责个人、品牌和产品的孵化判断；内容、受众、浏览器、媒体、发布和数据分析都是支撑能力。`mcn-incubation-core` 只是内部技术代号，不是最终产品名；当前产品契约和证据见 [`docs/mcn-incubation-v5`](docs/mcn-incubation-v5/README.md)。
> Lead 已接入一段极薄的营销内容世界思路：先辨认商业对象背后的世界，再按需向上抽象、向下拆分、横向展开或跨维连接；定位、内容和表现形式保持分开。首轮生产 Lead 真实试验技术 `2/2` 完成但业务未通过：水果生命周期只有局部正信号，黄金仍被误作语义中心，没有进入送礼与人情世界。
> A48 随后只在密封评测工具面移除 `incubation_context`：黄金首次明确“黄金只是载体”，水果也真正进入历史、流行文化和未来世界，但两例仍因三模板、事实补造和越界交付被拒绝。宽泛方法上下文已证实是重要干扰源，但不是唯一病根；生产方法系统尚未删除或改写。
> 第一条类型化知识链已经接入原运行时：Lead Agent 可以分别读取已认证用户的当前项目事实和建议性 MCN 方法卡。账本仍保留追加式历史，但被新证据替代的旧事实和其他用户项目不会进入模型上下文。
> 方法检索也会携带已复核的来源类型、适用范围、支持主张、限制和复核日期。M01 的确定性检索修正已经测试，但付费 v4 复测没有调用方法工具，业务评审再次失败。平台指南只是有范围的证据，不会被当成通用增长公式。
> 项目证据现已采用独立的 Owner 隔离追加式链路：Lead Agent 能读取快照来源、哈希、范围、局限、争议和过期提醒，但证据不会自动升级为事实，浏览器凭据也不会进入模型上下文。
> 账号拆解现已具备规范身份、透明采样、作品观察、媒体原子、模式假设和仅迁移功能的离线证据合同；它尚未连接真实浏览器，也没有被宣称为生产可用的账号分析功能。
> 评测链已经能让现有 Lead Agent 读取隔离的版本化项目，并密封配置工具面、脱敏后的实际工具轨迹、最终回答、用量和数据库哈希。M01 v3 与后续付费 v4 复测都技术完成但业务评审未通过；系统没有据此宣称孵化质量已经提升。
> 小规模付费模型预检使用 `cd backend && uv run python scripts/run_incubation_preflight.py --execute`；输入、输出、哈希、用量和失败都记录在本地未跟踪的 `.deer-flow/incubation-preflight/` 台账中。
> 小样本架构对照使用 `backend/scripts/run_incubation_micro_bakeoff.py`，必须显式选择案例、候选和付费调用上限；密封证据保存在 `.deer-flow/incubation-micro-bakeoff/`，不创建第二个 Agent 运行时。
