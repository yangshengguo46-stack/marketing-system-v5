...
新入口已经由离线测试证明能准备项目、运行事件收集器并密封证据。真实 `v1` 在首个工具往返前触发 12 图超步上限；`v2` 正确读取了项目事实、外部证据和方法，却在通用 `write_file` 交付前耗尽 50 图超步。两次都没有最终答案，暴露的是评测器预算、交付格式、流式参数和失败用量证据缺陷；它们不能用于判断孵化结果是否改善。完整证据见 `../evidence/2026-08-11-m01-agent-evaluation.md`。
...
Agent 的一个 trial 可能包含多次模型调用，所以 trial 上限不是“模型请求次数”，递归上限也不是精确人民币费用上限。模型调用上限能限制请求次数，仍不是精确货币上限；首次真实运行只能选择一个案例、一个 phase，并在运行后核对实际 Token。
...
- `v1` 证明 12 个图超步不足；`v2` 完成事实、证据和方法读取，但被通用文件交付动作耗尽 50 图超步。两次都没有最终判断，不能进入业务评审；`v3` 必须使用聊天文本交付、100 图超步和 6 次模型调用硬上限。
