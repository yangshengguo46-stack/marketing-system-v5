...
新入口已经由离线测试证明能准备项目、运行事件收集器并密封证据。首次真实 `agent-eval-m01-v1` 在首个工具往返前触发图超步上限，没有最终答案，暴露的是评测器把 12 个 LangGraph 超步误当作足够业务步数，以及流式参数没有合并；它不能用于判断模型是否会正确使用三个知识工具，更不能判断孵化结果是否改善。完整证据见 `../evidence/2026-08-11-m01-agent-evaluation.md`。
...
| 业务数据库 | 无评测项目 | 无 | 每个 trial 独立项目，共用单次运行的隔离 SQLite |
...
- `--max-agent-steps` 限制每个 LangGraph trial 的图超步，当前 25 节点 Lead 图要求 `40..100`，单例建议 50；
- `--execute` 明确确认会产生模型费用。

Agent 的一个 trial 可能包含多次模型调用，所以 trial 上限不是“模型请求次数”，递归上限也不是精确人民币费用上限。首次真实运行只能选择一个案例、一个 phase，并在运行后核对实际 Token；在提供方或运行时具备可靠的货币预算中止能力前，不得把参数描述成绝对费用上限。
...
- 首次真实运行只证明 12 个图超步不足；没有最终判断和完整工具结果，不能进入业务评审。修正后仍需使用新 run ID 重跑。
- 没有自动业务评分。下一次一例真实运行仍需人工逐条核对无依据身份、资产、效果、数字、表现形式持续性和变现闭环。
...
- 第一次真实 trial 只运行 `M01:initial`，关闭 thinking、subagent、skill 和 mutation，人工确认后再执行；不得自动扩大到 36 案例。
