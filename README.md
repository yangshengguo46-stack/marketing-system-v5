...
> Method retrieval also carries reviewed source type, applicability, supported claims, limitations, and refresh dates. The sealed M01 run exposed a broad-query recall and platform-source anchoring failure; its deterministic repair is tested, while the paid business retest remains pending. Platform guidance remains scoped evidence rather than a universal growth rule.
...
> The evaluation path runs the existing Lead Agent against isolated versioned projects and seals its configured tool schemas, redacted actual tool trajectory, final output, usage, and database hash. M01 v3 completed technically but failed business review; no later paid retest or business-quality claim has been made.
