...
> A47 records the first production-Lead trial of that compact contract. Both cases completed technically with 64,005 tokens and failed business review. Both independently retrieved the broad `content-engine-v1` and `incubation-model-v1` cards; renewed full-incubation pressure is a hypothesis for offline audit, not a proven root cause, and no further paid rerun is authorized.
