...
> 2026-08-11 后续：用户授权试做后，A42 已先落最小 `TerritoryCandidate` 与向后兼容持久化，并保持无生产入口。该顺序调整不代表方法候选胜出；A42 的 24 个密封试验反而否定了结构化必填输出、单张方法卡和单张机制卡，修正两遍法也仅部分命中。生产方法、知识和 Lead 提示词仍未改动。
...
5. 领域和持久化最小结构已作为无入口记录能力落地；任何生成方法、知识或两遍候选仍须先通过同标签异事实、新证据修订和事实边界评测，不能因黄金礼品一例局部改善就宣布通用。
