...
> [!NOTE]
> The current marketing-brain checkpoint keeps four evaluation utilities under `backend/scripts/`: the visible-answer baseline, a bounded content-world mapper, a business-semantic backbone parser, and the offline account-content structure evaluator. They require explicit `--execute` and write redacted artifacts under `.deer-flow/`. The default Lead can invoke two bounded capabilities on demand: `analyze_business_semantics` explicates what the user does, while `explore_content_worlds` expands one fact-bounded map of what that business can discuss long term. Business-operation design is outside both capabilities; the overall Lead may address it separately when the user's request requires it. The account-level candidate separates source object, audience world, content engines, and attention entries. It passed the reviewed medical-aesthetics and watch cases, but remains unregistered after the production content-world probes failed. Neither capability is middleware, a mandatory incubation stage, or the final decision-maker. Selector and multi-adviser experiments were rejected; their evidence remains in the ledger rather than in production code. See [`A34`](docs/mcn-incubation-v5/audits/A34-production-content-world-exploration.md), [`A42`](docs/mcn-incubation-v5/audits/A42-content-structure-and-commerce-boundary.md), and [`ADR-008`](docs/mcn-incubation-v5/decisions/ADR-008-content-structure-boundary.md).
