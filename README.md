...
> [!NOTE]
> The current marketing-brain checkpoint keeps three evaluation utilities under `backend/scripts/`: the visible-answer baseline, a bounded content-world mapper, and a business-semantic backbone parser. They require explicit `--execute` and write redacted artifacts under `.deer-flow/`. The default Lead can now invoke the same validated semantic contract on demand through `analyze_business_semantics`; it is not middleware, a mandatory incubation stage, or a final decision-maker. The other evaluators remain offline. Selector and multi-adviser experiments were rejected; their evidence remains in the ledger rather than in production code.
