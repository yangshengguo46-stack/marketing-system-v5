...
3. 第一候选是一张短、按需、版本化的方法卡；不改核心提示词，不建立固定四步流程，不使用关键词决定算子或行业答案。
...
7. `content-world-exploration-v1` 目前只存在于审计和 ADR 的候选命名中，不注册到 `methods.py`、`knowledge.py`、`incubation_context` 或 Lead 提示词。
