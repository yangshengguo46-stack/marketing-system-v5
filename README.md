...
> Method retrieval also carries reviewed source type, applicability, supported claims, limitations, and refresh dates. Platform guidance remains scoped evidence rather than a universal growth rule.
...
> The next evaluation path now runs the existing Lead Agent against isolated versioned projects and seals its configured tool schemas, redacted actual tool trajectory, final output, usage, and database hash. It remains offline-tested only; no new paid trial or business-quality claim has been made.
