...
> [!NOTE]
> The current marketing-brain checkpoint keeps three evaluation utilities under `backend/scripts/`: the visible-answer baseline, a bounded content-world mapper, and a business-semantic backbone parser. They require explicit `--execute` and write redacted artifacts under `.deer-flow/`. The default Lead can invoke two bounded capabilities on demand: `analyze_business_semantics` explicates what the user does, while `explore_content_worlds` expands one fact-bounded, rooted map of what that business can discuss long term. Neither capability is middleware, a mandatory incubation stage, or the final decision-maker. The offline evaluators remain unregistered. Selector and multi-adviser experiments were rejected; their evidence remains in the ledger rather than in production code. Live Gateway acceptance across seafood, gold gifts, a fruit store, and industrial protective packaging is recorded in [`A34`](docs/mcn-incubation-v5/audits/A34-production-content-world-exploration.md).
