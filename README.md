...
> [!IMPORTANT]
> This workspace is the fifth-version MCN incubation fork. DeerFlow's single Lead Agent is being specialized as the incubation decision-maker for people, brands, and products. Content, audience, browser, media, publishing, and analytics capabilities remain supporting tools. The internal package name `mcn-incubation-core` is not a final product name; current contracts and evidence live in [`docs/mcn-incubation-v5`](docs/mcn-incubation-v5/README.md).
> The Lead now carries a compact marketing-world thinking contract: identify the world behind a commercial object, then optionally abstract upward, decompose downward, expand horizontally, or connect across domains. Positioning, content, and expression form remain distinct. Its first two production-Lead trials completed technically but failed business review: fruit lifecycle was only a partial signal, while gold remained the semantic center instead of gift-giving and human relations.
> Its first typed knowledge path is now wired into the existing runtime: the Lead Agent can read the authenticated owner's current project truths separately from advisory MCN method cards. Append-only history remains in the ledger, while superseded facts and other users' projects are excluded from model context.
> Method retrieval also carries reviewed source type, applicability, supported claims, limitations, and refresh dates. The deterministic M01 retrieval repair is tested, but the paid v4 retest did not call the method tool and again failed business review. Platform guidance remains scoped evidence rather than a universal growth rule.
...
> The evaluation path runs the existing Lead Agent against isolated versioned projects and seals its configured tool schemas, redacted actual tool trajectory, final output, usage, and database hash. M01 v3, v4, and the two-trial v5 retest completed technically and failed business review; no business-quality claim has been made. Mutation evaluation now appends evidence to the same project and resumes the same thread instead of asking an isolated answer to invent a prior judgment.
...
> The audited content-world operator cards remain evaluation-only: they are not keyword-routed, not a mandatory workflow, and not stored in a new vector database. A46 separately installed a smaller, example-free thinking contract in the production Lead at the user's direction; it did not promote the rejected cards.
> A47 records the first production-Lead trial of that compact contract. A48 then removed `incubation_context` only from a sealed evaluation tool surface: gold first stated that gold was only the carrier, and fruit expanded into history, popular culture, and future worlds. The two-case ablation still failed business review, so broad method context is now a supported material interferer, not the sole cause; production method retrieval remains unchanged pending a compact task-relevant redesign.
...
Operators can set `subagents.allowed_agents` to expose only selected built-in or custom roles for a run configuration. `null` keeps the full discovered registry, while `[]` disables every role; the `task` tool checks the same allowlist before executing a requested sub-agent.
...
- Run background scheduled executions as non-interactive DeerFlow runs (`ask_clarification` and the subject-answer recorder are not exposed there)
