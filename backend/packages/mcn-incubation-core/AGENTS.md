...
Read `docs/mcn-incubation-v5/current/PRODUCT_CONTRACT.md` and the A20-A48 audits before changing this package.
...
- A48 upgrades broad method context from an inference to a supported material interferer, not the sole failure cause. Its `method_context_mode=disabled` switch is evaluator-only, content-world-only, and incompatible with subagents; it must not become a production keyword route or global method deletion. The no-method trial improved both anchors but remained business-rejected. Redesign task-relevant compact projection and the mother-contract boundary offline before another paid comparison.
