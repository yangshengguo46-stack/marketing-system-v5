...
> [!IMPORTANT]
> This local fifth-version branch keeps DeerFlow's runtime intact and targets a usable 80-point **marketing brain** for incubation: marketing subject, positioning, content world, presentation format, monetization, and fact boundaries. Media and account extraction are supporting evidence capabilities, not the product target. The isolated E15 experiment now includes a bounded account-link collection contract, content-addressed local snapshots, and a size-limited Lead projection; it is still not registered with the runtime and has not passed a real multi-video account acceptance. See [`ADR-007`](docs/mcn-incubation-v5/decisions/ADR-007-80-point-learning-loop.md), [`A35`](docs/mcn-incubation-v5/audits/A35-bounded-account-link-evidence.md), and the [`ledger`](docs/mcn-incubation-v5/LEDGER.md).
