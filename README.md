...
> [!IMPORTANT]
> This local fifth-version branch keeps DeerFlow's runtime intact and targets a usable 80-point **marketing brain** for incubation: marketing subject, positioning, content world, presentation format, monetization, and fact boundaries. Media, account extraction, and audience intelligence are supporting evidence capabilities, not the product target. The isolated E15 experiment includes bounded account-link evidence, a six-platform search contract, account-scoped local browser login state, an append-only audience observation ledger, and a checkpoint-bound ByteDance HLLM audience adapter. Douyin has passed one real third-party account acceptance: stable identity, author-qualified posts, two local media evidence records, ten pseudonymized visible interactions, and a bounded Lead projection. This remains a small partial sample, not full follower demographics or a causal success formula; the other platforms, live/commerce audiences, repeated actor-linked sequences, and real HLLM inference still require acceptance. E15 is not registered with the runtime. See [`ADR-007`](docs/mcn-incubation-v5/decisions/ADR-007-80-point-learning-loop.md), [`A39`](docs/mcn-incubation-v5/audits/A39-daneng-real-account-acceptance.md), and the [`ledger`](docs/mcn-incubation-v5/LEDGER.md).

> [!NOTE]
> The current marketing-brain checkpoint keeps five evaluation utilities under `backend/scripts/`: the visible-answer baseline, a bounded content-world mapper, a business-semantic backbone parser, an offline account-content structure evaluator, and an isolated layered-map evaluator. They require explicit `--execute` and write redacted artifacts under `.deer-flow/`. The default Lead can invoke two bounded capabilities on demand: `analyze_business_semantics` explicates what the user does, while `explore_content_worlds` expands one fact-bounded map of what that business can discuss long term. Business-operation design is outside both capabilities; the overall Lead may address it separately when the user's request requires it. The account-level candidate separates source object, audience world, content engines, and attention entries. The later one-call candidate also attached rooted expansion nodes, but passed only `1/6` cases in its frozen run, so neither candidate is registered and the production runtime source remains identical to commit `ca2af9f8`. Neither capability is middleware, a mandatory incubation stage, or the final decision-maker. Selector and multi-adviser experiments were rejected; their evidence remains in the ledger rather than in production code. See [`A34`](docs/mcn-incubation-v5/audits/A34-production-content-world-exploration.md), [`A43`](docs/mcn-incubation-v5/audits/A43-layered-content-map-competition.md), and [`ADR-009`](docs/mcn-incubation-v5/decisions/ADR-009-reject-single-call-layered-map.md).
