...
Audit A37 and proposed ADR-009 distinguish one incubation authority from one
agent invocation. DeerFlow's Lead remains the sole user-facing incubation
decision-maker; bounded read-only specialists may be evaluated through the
existing manager-style `task` tool, with no handoff, project writes, recursive
delegation, forced route, or second runtime. The first
`incubation-evidence-researcher` is injected only into a copied evaluation
config, allows the three incubation readers, disables skills, and requires
...
**Runtime role allowlist**: `subagents.allowed_agents` optionally narrows the built-in and custom roles exposed in the Lead prompt and accepted by `task`; `null` preserves the full discovered registry and `[]` disables every role. The task tool rechecks this list before executor creation, so an unlisted but installed role cannot be invoked by name while unknown-role diagnostics retain their existing meaning. This is an execution-authority boundary for isolated runs, not a semantic workflow gate.
**Embedded runtime config propagation**: `DeerFlowClient.stream()` must place its explicit `app_config` in the internal runtime context, matching the Gateway worker. Lead construction, prompt-visible roles, and delegated `task` execution must resolve the same immutable per-client configuration snapshot; never let a client-scoped role silently fall back to ambient process config.
