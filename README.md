...
> [!IMPORTANT]
> This local fifth-version branch keeps DeerFlow's runtime intact and targets a usable 80-point **marketing brain** for incubation: marketing subject, positioning, content world, presentation format, monetization, and fact boundaries. Media, account extraction, and audience intelligence are supporting evidence capabilities, not the product target. The isolated E15 experiment includes bounded account-link evidence, a six-platform search contract, account-scoped local browser login state, an append-only audience observation ledger, and a checkpoint-bound ByteDance HLLM audience adapter. Public account scale and sampled content response can be observed now; detailed demographics, live/commerce audiences, real actor-linked sequences, and real HLLM inference still require platform/provider acceptance. E15 is not registered with the runtime. See [`ADR-007`](docs/mcn-incubation-v5/decisions/ADR-007-80-point-learning-loop.md), [`A37`](docs/mcn-incubation-v5/audits/A37-cross-platform-search-and-local-session.md), [`A38`](docs/mcn-incubation-v5/audits/A38-audience-intelligence-and-hllm.md), and the [`ledger`](docs/mcn-incubation-v5/LEDGER.md).
