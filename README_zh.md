...
> 方法检索也会携带已复核的来源类型、适用范围、支持主张、限制和复核日期。密封的 M01 运行暴露了宽查询漏召回和平台来源锚定问题；确定性修正已经测试，付费业务复测仍未进行。平台指南只是有范围的证据，不会被当成通用增长公式。
...
> 评测链已经能让现有 Lead Agent 读取隔离的版本化项目，并密封配置工具面、脱敏后的实际工具轨迹、最终回答、用量和数据库哈希。M01 v3 技术完成但业务评审未通过；此后没有再次付费复测，也没有据此宣称孵化质量已经提升。
